...
const sendPasswordResetOTP = async (email, otp) => {
    const mailOptions = {
        ...
        html: `
            <div style="font-family: 'Segoe UI', Tahoma, sans-serif; max-width: 480px; margin: 0 auto; background: #0f172a; color: #e2e8f0; border-radius: 12px; overflow: hidden;">
                <div style="background: linear-gradient(135deg, #dc2626, #f97316); padding: 32px 24px; text-align: center;">
                    <h1 style="color: white; margin: 0; font-size: 1.8rem;">🎓 MentorMatch</h1>
                    <p style="color: rgba(255,255,255,0.85); margin-top: 8px; font-size: 0.9rem;">Password Reset</p>
                </div>
                <div style="padding: 32px 24px; text-align: center;">
                    <p style="font-size: 1rem; margin-bottom: 24px;">Your password reset code is:</p>
                    <div style="background: #1e293b; border: 2px dashed #f97316; border-radius: 12px; padding: 20px; display: inline-block;">
                        <span style="font-size: 2.5rem; letter-spacing: 12px; font-weight: 700; color: #fb923c;">${otp}</span>
                    </div>
                    <p style="font-size: 0.85rem; color: #94a3b8; margin-top: 24px;">This code expires in <strong>10 minutes</strong>.</p>
                    <p style="font-size: 0.8rem; color: #64748b; margin-top: 16px;">If you didn't request this, please ignore this email.</p>
                </div>
